...
func t_af2a() {
   let a = af2a("-", 45, 13, 27.2)
   vvd(a, -0.7893115794313644842, 1e-12, "af2a", "a")
}

func t_anp() {
   vvd(anp(-0.1), 6.183185307179586477, 1e-12, "anp", "")
}

func t_anpm() {
   vvd(anpm(-4.0), 2.283185307179586477, 1e-12, "anpm", "")
}
...
func t_apcg() {
   ...
   ehp[1] = -0.415395237
   ehp[2] = -0.180084014
   let astrom = apcg(date1, date2, ebpv, ehp)
...
func t_apci() {
   ...
   let y = -2.92808623e-5
   let s =  3.05749468e-8
   let astrom = apci(date1, date2, ebpv, ehp, x, y, s)
...
func t_apci13() {
   let date1 = 2456165.5
   let date2 = 0.401182685
   let (astrom, eo) = apci13(date1, date2)
...
func t_apco() {
   ...
   let astrom = apco(date1, date2, ebpv, ehp, x, y, s, theta, elong, phi, hm, xp, yp, sp, refa, refb)
...
func t_apco13() {
   ...
   let rh = 0.59
   let wl = 0.55
   let (astrom, eo) = apco13(utc1, utc2, dut1, elong, phi, hm, xp, yp, phpa, tc, rh, wl)
...
func t_apcs() {
   ...
   ehp[1] = -0.209215307
   ehp[2] = -0.0906996477
   let astrom = apcs(date1, date2, pv, ebpv, ehp)
...
func t_apcs13() {
   ...
   pv[1][1] = -455.021831
   pv[1][2] = 0.0266151194
   let astrom = apcs13(date1, date2, pv)
...
func t_apio() {
   ...
   let refa = 0.000201418779
   let refb = -2.36140831e-7
   let astrom = apio(sp, theta, elong, phi, hm, xp, yp, refa, refb)
...
func t_apio13() {
   ...
   let rh = 0.59
   let wl = 0.55
   let astrom = apio13(utc1, utc2, dut1, elong, phi, hm, xp, yp, phpa, tc, rh, wl)
...
func t_atccq() {
   let date1 = 2456165.5
   let date2 = 0.401182685
   let (astrom, _) = apci13(date1, date2)
   let rc = 2.71
   let dc = 0.174
   let pr = 1e-5
   let pd = 5e-6
...
func t_atco13() {
   ...
   let rh = 0.59
   let wl = 0.55
   let (aob, zob, hob, dob, rob, eo) = atco13(rc, dc, pr, pd, px, rv, utc1, utc2, dut1, elong, phi, hm, xp, yp, phpa, tc, rh, wl)
...
func t_aticqn() {
   ...
   let (rc, dc) = aticqn(ri, di, astrom, b)
...
func t_atio13() {
   ...
   let rh = 0.59
   let wl = 0.55
   let (aob, zob, hob, dob, rob) = atio13(ri, di, utc1, utc2, dut1, elong, phi, hm, xp, yp, phpa, tc, rh, wl)
...
func t_atoc13() {
   ...
   (rc, dc) = atoc13("R", ob1, ob2, utc1, utc2, dut1, elong, phi, hm, xp, yp, phpa, tc, rh, wl)
   vvd(rc, 2.709956744659136129, 1e-12, "atoc13", "R/rc")
   vvd(dc, 0.1741696500898471362, 1e-12, "atoc13", "R/dc")
   ob1 = -0.09247619879782006106
   ob2 = 0.1717653435758265198
   (rc, dc) = atoc13("H", ob1, ob2, utc1, utc2, dut1, elong, phi, hm, xp, yp, phpa, tc, rh, wl)
   ...
   (rc, dc) = atoc13 ("A", ob1, ob2, utc1, utc2, dut1, elong, phi, hm, xp, yp, phpa, tc, rh, wl)
...
func t_atoi13() {
   ...
   ob1 = 2.710085107986886201
   ob2 = 0.1717653435758265198
   (ri, di) = atoi13("R", ob1, ob2, utc1, utc2, dut1, elong, phi, hm, xp, yp, phpa, tc, rh, wl)
   vvd(ri, 2.710121574447540810, 1e-12, "atoi13", "R/ri")
   vvd(di, 0.1729371839116608778, 1e-12, "atoi13", "R/di")
   ob1 = -0.09247619879782006106
   ob2 = 0.1717653435758265198
   (ri, di) = atoi13("H", ob1, ob2, utc1, utc2, dut1, elong, phi, hm, xp, yp, phpa, tc, rh, wl)
   vvd(ri, 2.710121574448138676, 1e-12, "atoi13", "H/ri")
   vvd(di, 0.1729371839116608778, 1e-12, "atoi13", "H/di")
   ob1 = 0.09233952224794989993
   ob2 = 1.407758704513722461
   (ri, di) = atoi13("A", ob1, ob2, utc1, utc2, dut1, elong, phi, hm, xp, yp, phpa, tc, rh, wl)
...
func t_c2ixy() {
   ...
   let rc2i = c2ixy(2400000.5, 53736, x, y)
...
func t_cal2jd() {
   let (djm0, djm) = cal2jd(2003, 06, 01)
...
func t_d2dtf() {
	let (iy, im, id, ihmsf) = d2dtf("UTC", 5, 2400000.5, 49533.99999)
	viv(iy, 1994, "d2dtf", "y")
	viv(im, 6, "d2dtf", "mo")
	viv(id, 30, "d2dtf", "d")
...
func t_d2tf() {
   let (s, ihmsf) = d2tf(4, -0.987654321)
...
func t_dtdb() {
   let dtdb = dtdb(2448939.5, 0.123, 0.76543, 5.0123, 5525.242, 3190.0)
   vvd(dtdb, -0.1280368005936998991e-2, 1e-15, "dtdb", "")
}

func t_dtf2d() {
   let (u1, u2) = dtf2d("UTC", 1994, 6, 30, 23, 59, 60.13599)
...
func t_era00() {
   let era00 = era00(2400000.5, 54388.0)
   vvd(era00, 0.4022837240028158102, 1e-12, "era00", "")
}

func t_fad03() {
   vvd(fad03(0.80), 1.946709205396925672, 1e-12, "fad03", "")
}

func t_fae03() {
   vvd(fae03(0.80), 1.744713738913081846, 1e-12, "fae03", "")
}

func t_faf03() {
   vvd(faf03(0.80), 0.2597711366745499518, 1e-12, "faf03", "")
}

func t_faju03() {
   vvd(faju03(0.80), 5.275711665202481138, 1e-12, "faju03", "")
}

func t_fal03() {
   vvd(fal03(0.80), 5.132369751108684150, 1e-12, "fal03", "")
}

func t_falp03() {
   vvd(falp03(0.80), 6.226797973505507345, 1e-12, "falp03", "")
}

func t_fama03() {
   vvd(fama03(0.80), 3.275506840277781492, 1e-12, "fama03", "")
}

func t_fame03() {
   vvd(fame03(0.80), 5.417338184297289661, 1e-12, "fame03", "")
}

func t_fane03() {
   vvd(fane03(0.80), 2.079343830860413523, 1e-12, "fane03", "")
}

func t_faom03() {
   vvd(faom03(0.80), -5.973618440951302183, 1e-12, "faom03", "")
}

func t_fapa03() {
   vvd(fapa03(0.80), 0.1950884762240000000e-1, 1e-12, "fapa03", "")
}

func t_fasa03() {
   vvd(fasa03(0.80), 5.371574539440827046, 1e-12, "fasa03", "")
}

func t_faur03() {
   vvd(faur03(0.80), 5.180636450180413523, 1e-12, "faur03", "")
}

func t_fave03() {
   vvd(fave03(0.80), 3.424900460533758000, 1e-12, "fave03", "")
}
...
func t_fk425() {
   ...
   let p1950 = 0.134
   let v1950 = 8.7
   let (r2000, d2000, dr2000, dd2000, p2000, v2000) = fk425(r1950, d1950, dr1950, dd1950, p1950, v1950)
...
func t_fk45z() {
   ...
   let (r2000, d2000) = fk45z(r1950, d1950, bepoch)
...
func t_fk524() {
   ...
   let p2000 = 0.1559
   let v2000 = 86.87
   let (r1950, d1950, dr1950, dd1950, p1950, v1950) = fk524(r2000, d2000, dr2000, dd2000, p2000, v2000)
...
func t_fk52h() {
   ...
   let px5 =  0.379210
   let rv5 = -7.6
   let (rh, dh, drh, ddh, pxh, rvh) = fk52h(r5, d5, dr5, dd5, px5, rv5)
...
func t_fk54z() {
   ...
   let (r1950, d1950, dr1950, dd1950) = fk54z(r2000, d2000, bepoch)
...
func t_fk5hz() {
   let r5 =  1.76779433
   let d5 = -0.2917517103
   let (rh, dh) = fk5hz(r5, d5, 2400000.5, 54479.0)
...
func t_h2fk5() {
   ...
   let pxh =  0.379210
   let rvh = -7.6
   let (r5, d5, dr5, dd5, px5, rv5) = h2fk5(rh, dh, drh, ddh, pxh, rvh)
...
func t_hfk5z() {
   let rh =  1.767794352
   let dh = -0.2917512594
   let (r5, d5, dr5, dd5) = hfk5z(rh, dh, 2400000.5, 54479.0)
...
func t_moon98() {
   let pv = moon98(2400000.5, 43999.9)
...
func t_num00b() {
   let rmatn = num00b(2400000.5, 53736)
...
func t_num06a() {
   let rmatn = num06a(2400000.5, 53736)
...
func t_obl06() {
   vvd(obl06(2400000.5, 54388.0), 0.4090749229387258204, 1e-14, "obl06", "")
}
...
func t_p06e() {
   let (eps0, psia, oma, bpa, bqa, pia, bpia, epsa, chia, za, zetaa, thetaa, pa, gam, phi, psi) = p06e(2400000.5, 52541.0)
...
func t_plan94() {
   var pv = zpv()
   pv = plan94(2400000.5, -320000.0, 3)
...
func t_pmpx() {
   ...
   let pmt = 8.75
   let pob = [0.9, 0.4, 0.1]
   let pco = pmpx(rc, dc, pr, pd, px, rv, pmt, pob)
...
func t_pmsafe() {
   ...
   let ep2a = 2400000.5
   let ep2b = 51544.5
   let (ra2, dec2, pmr2, pmd2, px2, rv2) = pmsafe(ra1, dec1, pmr1, pmd1, px1, rv1, ep1a, ep1b, ep2a, ep2b)
...
func t_pn00() {
   ...
   let (epsa, rb, rp, rbp, rn, rbpn) = pn00(2400000.5, 53736.0, dpsi, deps)
...
func t_pn00a() {
   let (dpsi, deps, epsa, rb, rp, rbp, rn, rbpn) = pn00a(2400000.5, 53736.0)
...
func t_pr00() {
   let (dpsipr, depspr) = pr00(2400000.5, 53736)
...
func t_pvstar() {
   ...
   let (ra, dec, pmr, pmd, px, rv) = pvstar(pv)
...
func t_pvtob() {
   ...
   let sp = 1e-8
   let theta = 5.0
   let pv = pvtob(elong, phi, hm, xp, yp, sp, theta)
...
func t_s2pv() {
   let pv = s2pv(-3.21, 0.123, 0.456, -7.8e-6, 9.01e-6, -1.23e-5)
...
func t_sp00() {
   vvd(sp00(2400000.5, 52541.0), -0.6216698469981019309e-11, 1e-12, "sp00", "")
}
...
func t_starpm() {
   ...
   let px1 =   0.74723
   let rv1 = -21.6
   let (ra2, dec2, pmr2, pmd2, px2, rv2) = starpm(ra1, dec1, pmr1, pmd1, px1, rv1, 2400000.5, 50083.0, 2400000.5, 53736.0)
...
func t_starpv() {
   ...
   let px =   0.74723
   let rv = -21.6
   let pv = starpv(ra, dec, pmr, pmd, px, rv)
...
func t_tpxes() {
   ...
   let raz = 2.3
   let decz = 1.5
   let (xi, eta, status) = tpxes(ra, dec, raz, decz)
